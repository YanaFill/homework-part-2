...
def create_post_endpoint(post: PostCreate, author_id: GUID, db: Session = Depends(get_db)):
...
def read_post(post_id: GUID, db: Session = Depends(get_db)):
...
def update_post_endpoint(post_id: GUID, post: PostCreate, db: Session = Depends(get_db)):
