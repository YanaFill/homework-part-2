...
def get_student_profile(student_id: int):
    ...
    try:
        ...
        if student and student.profile:
            ...
            print(f"Профіль студента {student.first_name} {student.last_name}: Адреса - {profile.address}, Телефон - {profile.phone_number}")
        else:
            print(f"Профіль для студента з id {student_id} не знайдено")
    finally:
        session.close()
